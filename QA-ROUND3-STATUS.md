...
> **整改时间**：2026-09-19（柏林）
> **代码库**：`https://github.com/shichuhao/zug`，提交链 `3e030fe` → `184d7c8` → 本文件所在提交
...
| # | 项目 | 状态 | 证据 / 说明 |
|---|---|---|---|
| 1 | 注册/登录无限流 | ✅ **限流已有 + 本轮补图形验证码** | `RATE_LIMITS`：注册 10min/5、登录 5min/20。线上实测第 6 次注册 → **429**（QA 报告"6 次全 200"与本轮实测不符，应为时序早于 `0b088fc` 提交或测试未计 IP）。本轮补 QA 建议的第二条：**注册图形验证码**（`GET /api/captcha`，5 分钟 TTL、一次一用、点阵字形而非 `<text>` 明文 —— 否则脚本一条正则就能读走答案） |
...
| 4 | 分享链接无过期 | ✅ **代码已有 TTL + 本轮对用户可见** | `SHARE_TTL_MS` 默认 7 天，`loadShare()` 读时校验过期并删文件，启动 `pruneShares()`。本轮 POST/GET 响应新增 `expires_at` / `ttl_days`，并在分享弹层显示「该链接 N 天后失效（日期）」—— 过期机制不告诉用户，就等于过几天突然「站点坏了」 |
...
| 7 | 主查询 ≤5s | ⏸ **部分达成** | 已做：① 预热改走常驻 worker（模型已常驻，不再每趟 fork 3.4GB python），内存门槛分档 6000→1200MB —— 此前门槛常年不满足，预热几乎每轮被跳过，热门车次永远冷启动；② 全站响应 gzip（首页 31KB→**9.6KB**、app.js 249KB→**76KB**），且用**异步** `zlib.gzip`（第一版用 gzipSync，几百 KB 的 JSON 会同步压几十毫秒，高并发时等于给事件循环上锁）。**未达成**：冷门车次仍需 zugfinder 实时抓取（8 天 × 限流间隔），属数据源硬约束 |
| 8 | 评论字段名不兼容 | ✅ **本轮新增** | `firstDefined(content, text)`，两个字段名都收（content 优先）；回复同步支持 |
| 9 | 错误文案 i18n | ✅ **本轮统一（含收尾）** | 新增 `apiFail()`：`error` 恒为稳定错误码 `E_XXX`，`message` 为中文兜底；前端 `SERVER_ERR_CODES`（30 条）按码查 i18n，中文串映射保留为兜底，三语（zh/en/de）补齐缺失 key。**收尾**：服务端最后 11 处仍直出中文的错误（缺少参数 train/url/type/line、时刻表未加载、查询异常、影响数据加载失败、内容过大、记录不存在）全部改为错误码；`journey_link_expired` 补上前端映射与三语文案 |
...
| `/api/health` 看不到关键降级 | 新增两项自检：`image_exif`（`tools/strip_exif.py` 缺失时 EXIF/GPS 剥离会**静默失效**，这种事必须运维可见）与 `captcha`（当前开关状态） |

## 二·补、本轮自查发现并修掉的两个问题（都是本轮自己引入的）

| 问题 | 影响 | 处置 |
|---|---|---|
| 验证码答案以 `<text>` 明文返回 | SVG 里的文本节点可直接读出，脚本一条正则即可绕过验证码 —— 等于白做 | 改为 **5×7 点阵字形**（`CAPTCHA_GLYPHS`）渲染成矩形，客户端只拿到几何图形；已用浏览器渲染确认可读、`grep -c "<text"` = 0 |
| gzip 使用同步 `zlib.gzipSync` | 几百 KB 的 JSON 在主线程同步压缩，高并发时阻塞事件循环（自己引入的性能回归） | 改为异步 `zlib.gzip` + `sendBuffer()` 统一出口 |
...
# 2) 前端端到端冒烟（16 项：跨文件符号可用性 / 登录弹窗 / 图形验证码 / 弱密码拦截 /
